...
const Bestseller = () => {
 ...
 const scrollRef=useRef(null);
    ...
    useEffect(()=>{
      const fetchitems=async()=>{
       try{
        ...
        const data=req.data.filter((item)=>item.Bestselling===true);
  ...
  return (
    <div className='h-[30rem] md:h-[40rem] mt-10 lg:h-[42rem] lg:w-[95%] md:w-[95%]   flex flex-col justify-evenly text-white snap-start '>
      <a className='text-3xl md:text-5xl font-semibold font-[Jura] ml-8'>Bestseller</a>
      <div className='h-[80%]  w-[100%] relative flex justify-center items-center'>
        <div ref={scrollRef} className='h-[80%] w-[90%] lg:h-[90%]  overflow-x-scroll scroll-smooth  flex flex-row justify-evenly items-center gap-4 '> 
          {cards.map((item)=>(

...
const Braclet = () => {
    const [allbraclet,setbraclet]=useState([]);
const [displayedBraclet, setDisplayedBraclet] = useState([]);
...
const navigate=useNavigate();
    ...
    const getproducts=async()=>{
       ...
       const items=data.data.filter((item)=>item.category=='Braclets');
       setbraclet(items);
       setDisplayedBraclet(items)
       
    }

    useEffect(() => {
  let filtered = [...allbraclet];
  ...
  setDisplayedBraclet(filtered);
}, [allbraclet, sortType, availability]);
   
    useEffect(()=>{
       document.title="Braclets || Aaina"

    const metaDescription=document.querySelector('meta[name="description"]');
    if(metaDescription){
      metaDescription.setAttribute('content','Buy somee popular Braclets at Aaina store');
    }else{
      const newMeta=document.createElement('meta');
      newMeta.name='desciption';
      newMeta.content='Buy somee popular Braclets at Aaina store';
  ...
  return (
   ...
   <div className=" h-fit relative text-white mt-4 text-center flex flex-col items-center ">
    ...
    <h1 className="text-[43px] font-bold ">Braclet</h1>
...
<div className='h-[95%] mt-10  grid grid-cols-2 gap-4 lg:grid-cols-3'>
     {displayedBraclet.map((card,index)=>(

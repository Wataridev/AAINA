...
const Chains = () => {
const [allchains,setchains]=useState([]);
const [displayedchains, setDisplayedchains] = useState([]);
...
const navigate=useNavigate();
    ...
    const getproducts=async()=>{
       ...
       const items=data.data.filter((item)=>item.category=='Chains');
       setchains(items);
       setDisplayedchains(items)
  ...
  setDisplayedchains(filtered);
}, [allchains, sortType, availability]);
   
    useEffect(()=>{
       document.title="Chains || Aaina"

    const metaDescription=document.querySelector('meta[name="description"]');
    if(metaDescription){
      metaDescription.setAttribute('content','Buy somee popular Chains at Aaina store');
    }else{
      const newMeta=document.createElement('meta');
      newMeta.name='desciption';
      newMeta.content='Buy somee popular Chains at Aaina store';
  ...
  return (
    <div className=" h-fit text-white relative mt-[6rem] text-center flex flex-col items-center ">
         <button onClick={()=>navigate('/')} className='absolute left-5 cursor-pointer text-[28px]'>{"←"}</button>
    <h1 className="text-[43px] font-bold ">CHAINS</h1>
...
<div className='h-[95%] mt-10  grid grid-cols-2 gap-4 lg:grid-cols-3'>
     {displayedchains.map((card,index)=>(

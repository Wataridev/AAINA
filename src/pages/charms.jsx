...
const Charms = () => {
    const [allCharms,setCharms]=useState([]);
const [displayedCharms, setDisplayedCharms] = useState([]);
...
const navigate=useNavigate();
    ...
    const getproducts=async()=>{
       ...
       const items=data.data.filter((item)=>item.category=='Charms');
       setCharms(items);
       setDisplayedCharms(items)
...
}, [allCharms, sortType, availability]);
   
    useEffect(()=>{
       document.title="Charms || Aaina"

    const metaDescription=document.querySelector('meta[name="description"]');
    if(metaDescription){
      metaDescription.setAttribute('content','Buy somee popular Charms at Aaina store');
    }else{
      const newMeta=document.createElement('meta');
      newMeta.name='desciption';
      newMeta.content='Buy somee popular Charms at Aaina store';
  ...
  return (
    <div className=" relative h-fit text-white mt-4 text-center flex flex-col items-center ">
           <button onClick={()=>navigate('/')} className='absolute left-5 cursor-pointer text-[28px]'>{"←"}</button>
    <h1 className="text-[43px] font-bold ">Charms</h1>
...
<div className='h-[95%] mt-10  grid grid-cols-2 gap-4 lg:grid-cols-3'>
     {displayedCharms.map((card,index)=>(

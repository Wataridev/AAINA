...
const Earring = () => {
    const [allearrings,setearrings]=useState([]);
const [displayedEarrings, setdisplayedEarrings] = useState([]);
...
const navigate=useNavigate();
    ...
    const getproducts=async()=>{
       ...
       const items=data.data.filter((item)=>item.category=='Earrings');
       setearrings(items);
       setdisplayedEarrings(items)
       
    }

    useEffect(() => {
  let filtered = [...allearrings];
  ...
  setdisplayedEarrings(filtered);
}, [allearrings, sortType, availability]);
   
    useEffect(()=>{
       document.title="Earrings || Aaina"

    const metaDescription=document.querySelector('meta[name="description"]');
    if(metaDescription){
      metaDescription.setAttribute('content','Buy somee popular Earrings at Aaina store');
    }else{
      const newMeta=document.createElement('meta');
      newMeta.name='desciption';
      newMeta.content='Buy somee popular Earrings at Aaina store';
  ...
  return (
    <div className=" h-fit relative text-white mt-4 text-center flex flex-col items-center ">
          <button onClick={()=>navigate('/')} className='absolute left-5 cursor-pointer text-[28px]'>{"←"}</button>
    <h1 className="text-[43px] font-bold ">EARRINGS</h1>
...
<div className='h-[95%] mt-10  grid grid-cols-2 gap-4 lg:grid-cols-3'>
     {displayedEarrings.map((card,index)=>(
